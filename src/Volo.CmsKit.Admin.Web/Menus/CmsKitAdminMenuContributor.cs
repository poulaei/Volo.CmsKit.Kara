...
public class CmsKitAdminMenuContributor : IMenuContributor
{
    ...
    private Task AddCmsMenuAsync(MenuConfigurationContext context)
    {
        ...
        //Added By Poolaei @1402/12/06
        cmsMenus.Add(new ApplicationMenuItem(
               CmsKitAdminMenus.ImageGallery.ImageGalleryMenu,
               l["ImageGallery"],
               "/Cms/GalleryManagement",
               "fa fa-image",
               order: 8)
           .RequireFeatures(CmsKitFeatures.GalleryImageEnable)
           .RequireGlobalFeatures(typeof(GalleryImageFeature))
           .RequirePermissions(CmsKitAdminPermissions.GalleryImages.Default));

        cmsMenus.Add(new ApplicationMenuItem(
              CmsKitAdminMenus.ContentBoxes.ContentBoxMenu,
              l["ContentBox"],
              "/Cms/ContentBoxes",
              "fa  fa-list-alt",
              order: 8)
          .RequireFeatures(CmsKitFeatures.ContentBoxEnable)
          .RequireGlobalFeatures(typeof(ContentBoxFeature))
          .RequirePermissions(CmsKitAdminPermissions.ContentBoxes.Default));
