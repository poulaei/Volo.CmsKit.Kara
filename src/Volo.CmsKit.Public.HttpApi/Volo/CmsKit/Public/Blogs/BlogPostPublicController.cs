...
public class BlogPostPublicController : CmsKitPublicControllerBase, IBlogPostPublicAppService
{
    ...
    [HttpGet]
    [Route("post")]
    public virtual Task<BlogPostCommonDto> GetAsync(Guid id)
    {
         return BlogPostPublicAppService.GetAsync(id);
    }
    [HttpGet]
    [Route("posts")]
    public virtual Task<PagedResultDto<BlogPostCommonNoContentDto>> GetListWithOutContentAsync(BlogPostGetListInput input)
    {
        return BlogPostPublicAppService.GetListWithOutContentAsync(input);
    }
    [HttpGet]
    [Route("{blogSlug}")]
