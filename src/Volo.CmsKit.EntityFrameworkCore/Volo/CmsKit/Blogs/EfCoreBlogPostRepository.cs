...
public class EfCoreBlogPostRepository : EfCoreRepository<ICmsKitDbContext, BlogPost, Guid>, IBlogPostRepository
{
    ...
    public virtual async Task<List<BlogPost>> GetListWithOutContentAsync(
       string filter = null,
       Guid? blogId = null,
       Guid? authorId = null,
       string tagName = null,
...

    {
        ...
        {
            entityIdFilters = await _entityTagManager.GetEntityIdsFilteredByTagNameAsync(tagName, BlogPostConsts.EntityType, CurrentTenant.Id, cancellationToken);
        }
        ...
        return combinedResult.Select(s =>
        {
           ...
           // s.blogPost.Content = String.Empty();
            return s.blogPost;
        }).ToList();
    }
